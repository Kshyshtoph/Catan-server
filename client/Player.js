class Player {
  ...
  drawVictoryPoints = () => {
    ...
    if (
      currentLongestRoad.playerIndex ===
      players.findIndex(player => player === this)
    ) {
      sum += 2;
    }
    if (
      currentHighestKnightPower.playerIndex ===
      players.findIndex(player => player === this)
    ) {
      sum += 2;
    }
  ...
  findLongestRoad = () => {
    ...
    for (let i = 0; i < roadsInPlay.length; i++) {
      map.forEach(node => {
        if (
          roadsInPlay[i].x <= node.x + 50 &&
          roadsInPlay[i].x >= node.x - 50 &&
          roadsInPlay[i].y <= node.y + 50 &&
          roadsInPlay[i].y >= node.y - 50 &&
  ...
  drawKnightsPlayed = () => {
    if (this.knightsPlayed > currentHighestKnightPower.number) {
      ...
      currentHighestKnightPower.playerIndex = players.findIndex(
        player => player === this
      );
    }
    ctx.font = "30px Arial";
